(ns ansys.main
  "Kotodama WASM entrypoint for the Ansys clean-room actor (L4) — Clojure port.
  ...
  facts (`ansys.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"ansys.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ansys-compat" :tier tier :entities entities} 200])
